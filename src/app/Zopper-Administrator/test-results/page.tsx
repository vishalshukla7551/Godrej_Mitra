  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // If API fails, we'll fall back to mock data below
      }

      if (!data || data.length === 0) {
        data = MOCK_SUBMISSIONS;
      }
      if (!statistics || !statistics.totalSubmissions) {
        statistics = MOCK_STATS as typeof stats;
      }
