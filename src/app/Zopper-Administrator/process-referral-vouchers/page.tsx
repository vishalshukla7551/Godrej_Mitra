...
export default function ProcessReferralVouchers() {
  const [file, setFile] = useState<File | null>(null);

  return (
    <div className="space-y-6 p-6">
      <h1 className="text-2xl font-bold text-gray-900">Process Referral Vouchers</h1>
      
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h3 className="font-semibold text-blue-900 mb-2">📝 Instructions:</h3>
        <ol className="text-sm text-blue-800 space-y-1 list-decimal ml-4">
          <li>Export Excel from Referrals page</li>
      ...
      {file && (
        <button onClick={() => alert('Mock: Processing referral vouchers')} className="w-full py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">
          Process File
        </button>
      )}
    </div>
