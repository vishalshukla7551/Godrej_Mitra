...
interface ABMProfileApiResponse {
  success: boolean;
  data?: { abm: { id: string; fullName: string; phone: string }; stores: any[] };
  error?: string;
}
...
export default function ABMPage() {
  const [abmName, setAbmName] = useState('User');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchABMProfile = async () => {
      try {
        const res = await fetch('/api/abm/profile');
        if (res.ok) {
          const json = (await res.json()) as ABMProfileApiResponse;
          if (json.success && json.data?.abm?.fullName) {
            const firstName = json.data.abm.fullName.split(' ')[0];
            setAbmName(firstName.charAt(0).toUpperCase() + firstName.slice(1).toLowerCase());
          }
        }
      } catch (error) { console.error('Error fetching ABM profile:', error); }
  ...
  const cards = [
    { title: 'View Reports', description: 'Check your stores performance', href: '/ABM/report', color: 'blue' as const, icon: <ReportIcon /> },
    { title: 'Store Settings', description: 'Manage your store configuration', href: '/ABM/profile', color: 'orange' as const, icon: <SettingsIcon /> },
  ];
