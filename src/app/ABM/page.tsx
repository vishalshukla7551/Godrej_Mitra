      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.success && json.data?.abm?.fullName) {
            ...
            setAbmName(properCaseFirstName);
          }
        }
      } catch (error) {
        console.error('Error fetching ABM profile:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex-1 relative overflow-hidden bg-gray-900">
      ...
      <div className="flex justify-between items-center px-10 pt-6">
        <div>
          <p className="text-neutral-50 text-3xl font-semibold leading-[48px]">
            {loading ? 'Hello User,' : `Hello ${abmName},`}
          </p>
      ...
      <section className="mt-12 px-10 pb-10">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 max-w-4xl">
          ...
          {/* Store Settings card */}
          <Link href="/ABM/profile" className="relative h-60 rounded-2xl border border-zinc-100 bg-white shadow-[0_6px_24px_rgba(0,0,0,0.12)] p-6 block hover:shadow-[0_8px_32px_rgba(0,0,0,0.16)] transition-shadow cursor-pointer">
            {/* Arrow button - top right */}
