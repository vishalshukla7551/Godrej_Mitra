...
export default function ProfilePage() {
  ...
  const fetchKycInfo = async () => {
    try {
      const response = await fetch('/api/canvasser/kyc/info');
      if (response.ok) {
  ...
  const fetchStoreInfo = async () => {
    try {
      const response = await fetch('/api/canvasser/profile');
      if (response.ok) {
        const data = await response.json();
      ...
      const requestRes = await fetch('/api/canvasser/store-change-request');
      if (requestRes.ok) {
  ...
  const fetchAllStores = async (search = '') => {
    try {
      ...
      const res = await fetch(`/api/canvasser/stores?${params}`);
      if (res.ok) {
  ...
  const handleSubmitStoreChangeRequest = async () => {
    ...
    try {
      const res = await fetch('/api/canvasser/store-change-request', {
        method: 'POST',
        headers: {
  ...
  const handlePersonalInfoSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/canvasser/profile/update', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleKYCSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/canvasser/kyc/verify-pan', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-medium ${pendingRequest.status === 'PENDING' ? 'text-yellow-800' :
                          ...
                          Change Request {pendingRequest.status}
                        </span>
                      </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full max-h-[80vh] overflow-hidden">
            <div className="p-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-bold text-gray-900">Change Store</h3>
                <button
            ...
            <div className="p-4 overflow-y-auto max-h-[60vh]">
              ...
              {currentStore && (
                <div className="mb-4">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                    <div className="text-sm font-semibold text-gray-900">{currentStore.name}</div>
                    {currentStore.city && (
                      <div className="text-xs font-medium text-gray-700">{currentStore.city}</div>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm text-gray-900 placeholder:text-gray-500"
                />
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="max-h-48 overflow-y-auto border border-gray-300 rounded-lg">
                  {allStores.length === 0 ? (
                    <div className="p-4 text-center text-gray-700 text-sm font-medium">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className={`text-sm font-semibold ${isSelected ? 'text-blue-900' :
                              isCurrent ? 'text-gray-600' : 'text-gray-900'
                              }`}>
                              {store.name}
                              {isCurrent && (
                              ...
                              {isSelected && !isCurrent && (
                                <span className="ml-2 text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded-full font-medium">
                                  Selected
                                </span>
                              )}
                            </div>
                            {(store.city) && (
                              <div className={`text-xs font-medium ${isSelected ? 'text-blue-700' :
              ...
              <div className="mb-4">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none text-sm text-gray-900 placeholder:text-gray-500"
                />
              </div>
            ...
            <div className="p-4 border-t border-gray-200 flex gap-3 justify-end">
              <button
                onClick={() => setShowStoreChangeModal(false)}
                className="px-4 py-2 text-gray-800 font-medium border border-gray-300 rounded-lg hover:bg-gray-50 text-sm"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 bg-black text-white font-medium rounded-lg hover:bg-gray-900 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
