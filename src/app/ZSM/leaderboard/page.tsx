...
export default function LeaderboardPage() {
  ...
  const fetchLeaderboard = async (monthStr: string) => {
    try {
      ...
      const res = await fetch(`/api/zsm/leaderboard?month=${monthIndex}&year=${year}&limit=20`);
  ...
  const podiumData = leaderboardData?.zsms.slice(0, 3) || [];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 px-6 py-6">
      ...
      <div className="text-center mb-3">
        <h1 className="text-3xl font-bold text-white">ZSM Ka Mahasangram</h1>
      </div>

      <div className="text-center mb-4">
        <p className="text-neutral-300 text-sm">Compete with other ZSMs based on your region&apos;s performance</p>
      </div>
        ...
        <>
          {reorderedPodium.length > 0 && (
            <div className="flex justify-center items-end gap-4 mb-8 max-w-5xl mx-auto">
              {reorderedPodium.map((zsm) => {
                const isChampion = zsm.rank === 1;
                const bgGradient = zsm.rank === 1 ? 'from-yellow-400 to-yellow-600' : zsm.rank === 2 ? 'from-gray-400 to-gray-500' : 'from-orange-400 to-orange-600';
                const height = isChampion ? 'h-52' : zsm.rank === 2 ? 'h-44' : 'h-40';
                
                return (
                  <div key={zsm.zsmId} className="relative">
                    {isChampion && (
                    ...
                    <div className={`w-64 ${height} rounded-2xl bg-gradient-to-br ${bgGradient} shadow-xl p-5 flex flex-col justify-between ${zsm.isCurrentUser ? 'ring-2 ring-white' : ''}`}>
                      <div className="flex justify-between items-start">
                        <div className="w-12 h-12 rounded-xl bg-white/20 flex items-center justify-center">
                          <span className="text-2xl">{zsm.rank === 1 ? '🏆' : zsm.rank === 2 ? '🥈' : '🥉'}</span>
                        </div>
                        {zsm.isCurrentUser && (
                      ...
                      <div>
                        <p className="text-white text-sm font-semibold mb-0.5 truncate">{zsm.zsmName}</p>
                        <p className="text-white/80 text-xs mb-2">{zsm.storeCount} stores • {zsm.region || 'N/A'}</p>
                        <div className="text-white text-xl font-bold">{zsm.totalIncentive}</div>
                      </div>
                      <div className="absolute bottom-3 right-3 bg-white/20 rounded-lg px-2.5 py-0.5">
                        <span className="text-white text-xs font-bold">{isChampion ? 'CHAMPION' : `#${zsm.rank}`}</span>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className="rounded-2xl bg-purple-600 overflow-hidden">
                <div className="px-5 py-3">
                  <div className="flex items-center gap-2 text-white">
                    ...
                    <h2 className="text-lg font-bold">All ZSM Rankings</h2>
                  </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-700">
                        ...
                        <th className="text-left px-4 py-3 text-neutral-400 text-xs font-medium">ZSM Name</th>
                    ...
                    <tbody>
                      {leaderboardData.zsms.map((zsm) => (
                        <tr
                          key={zsm.zsmId}
                          className={`border-b border-gray-800 ${
                            zsm.isCurrentUser ? 'bg-gradient-to-r from-blue-500/20 to-transparent' :
                            zsm.rank <= 3 ? 'bg-gradient-to-r from-yellow-500/10 to-transparent' : ''
                          }`}
                        >
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              {zsm.rank <= 3 && <span className="text-xl">{zsm.rank === 1 ? '👑' : zsm.rank === 2 ? '🥈' : '🥉'}</span>}
                              {zsm.rank > 3 && <span className="text-white font-semibold text-sm">#{zsm.rank}</span>}
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <div>
                              <div className="text-white text-sm font-medium flex items-center gap-2">
                                {zsm.zsmName}
                                {zsm.isCurrentUser && <span className="px-1.5 py-0.5 bg-blue-500 text-white text-xs rounded">YOU</span>}
                              </div>
                              <div className="text-neutral-400 text-xs">{zsm.region || 'N/A'}</div>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className="text-white text-sm">{zsm.storeCount}</span>
                          </td>
                          <td className="text-right px-4 py-3">
                            <div className="text-white text-sm">{zsm.adldUnits}</div>
                            <div className="text-neutral-400 text-xs">{zsm.adldRevenue}</div>
                          </td>
                          <td className="text-right px-4 py-3">
                            <div className="text-white text-sm">{zsm.comboUnits}</div>
                            <div className="text-neutral-400 text-xs">{zsm.comboRevenue}</div>
                          </td>
                          <td className="text-right px-4 py-3">
                            <div className="text-green-500 font-bold text-base">{zsm.totalIncentive}</div>
                            <div className="text-neutral-400 text-xs">{zsm.totalSales} sales</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
