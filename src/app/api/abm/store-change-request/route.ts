...
// GET /api/abm/store-change-request
// Get pending store change request for the authenticated ABM
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('Error in GET /api/abm/store-change-request', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}

// POST /api/abm/store-change-request
...
export async function POST(req: NextRequest) {
  try {
    ...
    const storeChangeRequest = {
      ...
      requestedStoreIds,
      currentStoreIds: user.abmProfile.storeIds || [],
      reason: reason || null,
      status: 'PENDING',
      createdAt: new Date().toISOString(),
      abmId: user.abmProfile.id,
      abmName: (user.abmProfile as any)?.fullName || 'Unknown'
    };
    ...
    console.error('Error in POST /api/abm/store-change-request', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}