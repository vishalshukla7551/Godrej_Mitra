...
// POST /api/ase/kyc/verify-pan
// Verifies PAN using Karza API and updates ASE user's name
export async function POST(req: NextRequest) {
  try {
    const cookies = await (await import('next/headers')).cookies();
    const authUser = await getAuthenticatedUserFromCookies(cookies as any);

    if (!authUser || authUser.role !== ('ASE' as Role)) {
    ...
    try {
      ...
      console.log('Making Karza API request for ASE:', {
      ...
      if (process.env.KARZA_MOCK_MODE === 'true') {
        console.log('Using mock Karza API response for ASE development');
        ...
        console.log('Mock Karza API response for ASE:', panData);
        ...
        // Update ASE user's name and store KYC info in User metadata
        ...
        // Update ASE profile name and User metadata
        ...
        return NextResponse.json({
          ...
          kycInfo: kycInfo,
          aseUser: {
            id: user.aseProfile.id,
            phone: user.aseProfile.phone,
            fullName: cleanedFullName,
          },
          mockMode: true
        });
      ...
      console.log('Karza API response status for ASE:', karzaResponse.status);

      if (!karzaResponse.ok) {
        const errorData = await karzaResponse.json().catch(() => null);
        console.error('Karza API error response for ASE:', errorData);
        return NextResponse.json(
          { 
      ...
      console.log('Karza API success response for ASE:', panData);

      // Check if the API response is successful
      if (!panData.result) {
        console.error('No result in Karza API response for ASE:', panData);
        return NextResponse.json(
      ...
      console.error('Karza API Error for ASE:', apiError);
    ...
    console.error('Error in POST /api/ase/kyc/verify-pan', error);
    return NextResponse.json(
