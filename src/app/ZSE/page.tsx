...
interface ZSEProfileApiResponse {
  success: boolean;
  data?: { zse: { id: string; fullName: string; phone: string }; stores: any[] };
  error?: string;
}
...
export default function ZSEPage() {
  const [zseName, setZseName] = useState('User');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchZSEProfile = async () => {
      try {
        const res = await fetch('/api/zse/profile');
        if (res.ok) {
          const json = (await res.json()) as ZSEProfileApiResponse;
          if (json.success && json.data?.zse?.fullName) {
            const firstName = json.data.zse.fullName.split(' ')[0];
            setZseName(firstName.charAt(0).toUpperCase() + firstName.slice(1).toLowerCase());
          }
        }
      } catch (error) { console.error('Error fetching ZSE profile:', error); }
  ...
  return <ChristmasDashboard userName={zseName} loading={loading} cards={cards} />;
}
