...
/**
 * PRODUCTION SAFE SEC TO CANVASSER MIGRATION SCRIPT
 * 
 * This script safely migrates SEC user data to the Canvasser model
 * without losing any production data.
 * 
 * SAFETY FEATURES:
...
const LOG_FILE = `${BACKUP_DIR}/migration-${new Date().toISOString().replace(/[:.]/g, '-')}.log`;
...
function loadSecData() {
  try {
    ...
    log(`Loaded ${secUsers.length} SEC users from JSON file`);
    return secUsers;
  } catch (error) {
    log(`Error loading SEC data: ${error.message}`, 'ERROR');
    throw error;
  }
}
...
function transformSecToCanvasser(secUser) {
  ...
  const fullName = secUser.name || null;
  const employeeId = secUser.secId || null;
  const lastLoginAt = secUser.lastLoginAt ? new Date(secUser.lastLoginAt.$date || secUser.lastLoginAt) : null;
  const createdAt = secUser.createdAt ? new Date(secUser.createdAt.$date || secUser.createdAt) : new Date();
  const updatedAt = secUser.updatedAt ? new Date(secUser.updatedAt.$date || secUser.updatedAt) : new Date();
...
async function migrateSecToCanvasser() {
  ...
  try {
    log('=== SEC TO CANVASSER MIGRATION STARTED ===');
    log(`Mode: ${DRY_RUN ? 'DRY RUN' : 'LIVE MIGRATION'}`);
    
    // Load SEC data
    const secUsers = loadSecData();
...
async function main() {
  ...
  try {
    if (command === 'rollback') {
      ...
      if (!backupFile) {
        console.error('Usage: node migrate-sec-to-canvasser.js rollback <backup-file>');
        process.exit(1);
      }
