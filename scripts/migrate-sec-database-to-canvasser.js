...
/**
 * MIGRATE SEC DATABASE TO CANVASSER SCRIPT
 * 
 * This script:
 * 1. Deletes all existing Canvasser records
 * 2. Migrates all 35 SEC records from database to Canvasser collection as-is
 * 3. No updates, just direct migration
 * 
 * SAFETY FEATURES:
...
const LOG_FILE = `${BACKUP_DIR}/sec-to-canvasser-migration-${new Date().toISOString().replace(/[:.]/g, '-')}.log`;
...
// Transform SEC user to Canvasser format (as-is migration)
function transformSecToCanvasser(secUser) {
  // Convert MongoDB ObjectId to string if needed
  const id = secUser._id ? secUser._id.toString() : undefined;
  ...
  return {
    // Use existing ID if available, otherwise let Prisma generate
    ...(id && { id }),
    fullName: secUser.name || secUser.fullName || null,
    phone: secUser.phone,
    employeeId: secUser.secId || secUser.employeeId || null,
    email: secUser.email || null,
    kycInfo: secUser.kycInfo || null,
    lastLoginAt: parseDate(secUser.lastLoginAt),
    storeId: secUser.storeId || null,
    city: secUser.city || null,
    AgencyName: secUser.AgencyName || null,
    AgentCode: secUser.AgentCode || null,
    createdAt: parseDate(secUser.createdAt) || new Date(),
    updatedAt: parseDate(secUser.updatedAt) || new Date(),
  };
}
...
async function migrateSecToCanvasser() {
  ...
  try {
    ...
    // Step 1: Delete all existing Canvasser records
    log('Step 1: Deleting all existing Canvasser records...');
    if (!DRY_RUN) {
      const deleteResult = await prisma.canvasser.deleteMany();
      stats.deletedCanvassers = deleteResult.count;
      log(`Deleted ${stats.deletedCanvassers} existing Canvasser records`);
    } else {
      const existingCount = await prisma.canvasser.count();
      stats.deletedCanvassers = existingCount;
      log(`Would delete ${stats.deletedCanvassers} existing Canvasser records`);
    }
    
    // Step 2: Migrate SEC users to Canvasser
    log('Step 2: Migrating SEC users to Canvasser collection...');
    for (let i = 0; i < secUsers.length; i++) {
      ...
      try {
        ...
        log(`Migrating ${i + 1}/${secUsers.length}: ${secUser.phone} (${secUser.name || 'No name'})`);
    ...
    log(`Total SEC users: ${stats.totalSec}`);
    log(`Deleted Canvassers: ${stats.deletedCanvassers}`);
    log(`Migrated SEC users: ${stats.migratedSec}`);
...
async function main() {
  ...
  try {
    if (command === 'rollback') {
      ...
      if (!backupFile) {
        console.error('Usage: node migrate-sec-database-to-canvasser.js rollback <backup-file>');
        process.exit(1);
      }
